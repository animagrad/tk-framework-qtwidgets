...
class NoteWidget(ActivityStreamBaseWidget):
    ...
    def apply_thumbnail(self, data):
        ...
        if (
            thumbnail_type == ActivityStreamDataHandler.THUMBNAIL_CREATED_BY
            and activity_id == self.activity_id
        ):
            thumb = utils.create_round_thumbnail(image)
            self.ui.user_thumb.setPixmap(thumb)

        elif (
            thumbnail_type == ActivityStreamDataHandler.THUMBNAIL_ATTACHMENT
            and activity_id == self.activity_id
        ):
    ...
    def add_replies(self, replies_and_attachments):
        ...
        for item in replies_and_attachments:
            ...
            if (
                ...
                and item["this_file"]["link_type"] == "upload"
            ):
    ...
    def _add_attachment_group(self, attachments, after_note):
        ...
        offset = (
            attachment_group.OFFSET_NONE
            if after_note
            else attachment_group.OFFSET_SMALL_THUMB
        )
    ...
    def set_note_info(self, data):
        ...
        entity_url = self._generate_entity_url(
            data["user"], this_syntax=False, display_type=False
        )
